...
void compute(std::vector<int> arr, int inp1, int inp2)
{
    int i = 0;
    bool valnum = 0;
    while (i < arr.size())
    {
        ...
        switch(arr[i] % 100)
        {
            ...
            case 3:
                ...
                if (!valnum)
                {
                    arr[parameters[0]] = inp1;
                    valnum = 1;
                }
                else
                {
                    arr[parameters[0]] = inp2;
                    valnum = 0;
                }
                i += 2;
                break;
...
int get_largest_thrust_signal(std::vector<int> main, int amps)
{
    int max = 0;
    int *p_settings = (int *) calloc(amps, sizeof(int));
    ...
    do
    {
        ...
        for (int i = 0; i < amps; i++)
        {
            compute(main, p_settings[i], prev_val);
        }
        max = prev_val > max ? prev_val : max;
        // I love c++ for this thing right here, this thing, INCLUDED IN THE STANDARD LIB
    } while (std::next_permutation(p_settings, p_settings + amps));
    return max;
}
...
int main()
{
    ...
    arr = parse(opcode);
    std::cout << "Enter the number of amplifiers: ";
    std::cin >> amps;
    std::cout << "[P1] The highest signal that can be sent to the thrusters is " << get_largest_thrust_signal(arr, amps) << std::endl;
    file.close();
    return 0;
}
